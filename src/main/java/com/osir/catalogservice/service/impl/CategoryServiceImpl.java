...
import com.osir.catalogservice.mapper.CategoryMapper;
...
//        category.setUpdateUser(BaseContext.getCurrentId());
    ...
    public void deleteById(Long id) {
        ...
        if(count > 0){
            //当前分类下有菜品，不能删除
            throw new DeletionNotAllowedException(ErrorMessageConstant.CATEGORY_BE_RELATED_BY_DISH);
        }
        ...
        if(count > 0){
            //当前分类下有菜品，不能删除
            throw new DeletionNotAllowedException(ErrorMessageConstant.CATEGORY_BE_RELATED_BY_SETMEAL);
        }
